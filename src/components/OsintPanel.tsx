...
function OsintPanelInner({ isMobile, onSweepVisualize, onScanGeolocate }: OsintPanelProps) {
  ...
  const TABS = [
    { id: 'scanner', label: t('PORT SCAN'), icon: Radar, placeholder: t('IP or hostname'), color: '#00E5FF' },
    { id: 'vuln', label: t('VULN SWEEP'), icon: Bug, placeholder: t('IP or hostname'), color: '#FF3D3D' },
    { id: 'dns', label: t('DNS'), icon: Server, placeholder: t('Domain name'), color: '#448AFF' },
    { id: 'whois', label: t('WHOIS'), icon: FileText, placeholder: t('Domain name'), color: '#FFD700' },
    { id: 'certs', label: t('CERTS'), icon: Lock, placeholder: t('Domain name'), color: '#E040FB' },
    { id: 'threats', label: t('THREATS'), icon: AlertTriangle, placeholder: t('IP, domain, or hash'), color: '#FF9500' },
    { id: 'headers', label: t('HEADERS'), icon: Code, placeholder: t('URL to inspect'), color: '#87CEEB' },
    { id: 'ssl', label: t('SSL/TLS'), icon: Shield, placeholder: t('Domain name'), color: '#76FF03' },
    { id: 'subdomains', label: t('SUBDOMAINS'), icon: Layers, placeholder: t('Domain to enumerate'), color: '#00BCD4' },
    { id: 'tech', label: t('TECH DETECT'), icon: Code, placeholder: t('URL to fingerprint'), color: '#9C27B0' },
    { id: 'shodan', label: t('SHODAN IOT'), icon: Network, placeholder: t('IP address'), color: '#FF3D3D' },
    { id: 'bgp', label: t('BGP ROUTE'), icon: Globe, placeholder: t('IP or ASN'), color: '#00E5FF' },
    { id: 'mac', label: t('MAC ADDR'), icon: Fingerprint, placeholder: t('MAC address'), color: '#FFD700' },
    { id: 'phone', label: t('PHONE INTEL'), icon: Phone, placeholder: t('Phone number (e.g. +1...)'), color: '#FF9500' },
    { id: 'leaks', label: t('DATA LEAKS'), icon: ShieldAlert, placeholder: t('Email address'), color: '#E040FB' },
    { id: 'github', label: t('GITHUB RECON'), icon: Terminal, placeholder: t('GitHub username'), color: '#87CEEB' },
    { id: 'sweep', label: t('IP SWEEP'), icon: Crosshair, placeholder: t('Enter IP address (e.g. 8.8.8.8)'), color: '#FF3D3D' },
  ];
      ...
      fetch('/api/geo')
        ...
        .then(geo => {
          ...
          if (geo.status === 'success' && geo.lat && geo.lon && onScanGeolocate) {
            ...
            setError(t("Could not retrieve your IP location."));
          }
        })
        .catch(err => {
          setLoading(false);
          setError(t("Network error:") + " " + err.message);
        });
    };
  ...
  const runLookup = useCallback(async () => {
    ...
    if (activeTab === 'sweep') {
      ...
      try {
        ...
        if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error || `${t('Sweep failed')} (${res.status})`); }
    ...
    try {
      ...
      if (activeTab === 'shodan' && res.status === 404) {
        setResults({ ip: query, status: t('No Shodan InternetDB records found'), ports: [], cpes: [], hostnames: [], tags: [], vulns: [] });
        setLoading(false);
        return;
      }
      ...
      if (res.ok) {
        ...
        setError(data.error || t('Lookup failed'));
      }
      ...
      <div className="mb-2 px-2 py-2 rounded border border-red-500/40 bg-red-500/15">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-[10px] font-mono font-bold text-red-400 tracking-wider">
            {t('SANCTIONED')} — {match.source || 'OFAC SDN'}
          </span>
        </div>
    ...
    if (activeTab === 'scanner') {
      ...
      return (
        <div>
          ...
          <ResultRow label={t('Target')} value={host} color="#00E5FF" />
          <ResultRow label={t('Scan Type')} value={r.scan_type || scanType} />
          <ResultRow label={t('Duration')} value={r.duration || r.scan_time} />
          {Array.isArray(ports) && ports.length > 0 && (
            <>
              <SectionHeader title={`${t('OPEN PORTS')} (${ports.length})`} icon={Wifi} color="#00E676" />
    ...
    if (activeTab === 'vuln') {
      ...
      return (
        <div>
          ...
          <ResultRow label={t('Target')} value={r.target || query} color="#FF3D3D" />
          <ResultRow label={t('Total CVEs')} value={Array.isArray(vulns) ? vulns.length : 0} color={Array.isArray(vulns) && vulns.length > 0 ? '#FF3D3D' : '#00E676'} />
          <ResultRow label={t('Risk Level')} value={r.risk_level || r.severity} />
                ...
                {exploits.slice(0, 10).map((e: any, i: number) => (
                  <div key={i} className="p-2 rounded-lg border border-orange-500/30 bg-orange-500/10 flex flex-col">
                    ...
                    <div className="text-[9px] font-mono text-[var(--text-muted)] mt-1 flex justify-between">
                      <span>{t('Source')}: {e.type?.toUpperCase() || 'UNKNOWN'}</span>
    ...
    if (activeTab === 'dns') {
      return (
        <div>
          ...
          <ResultRow label={t('Domain')} value={r.domain || query} color="#448AFF" />
          {r.A && <ResultRow label={t('A Records')} value={Array.isArray(r.A) ? r.A.join(', ') : r.A} />}
          {r.AAAA && <ResultRow label={t('AAAA')} value={Array.isArray(r.AAAA) ? r.AAAA.join(', ') : r.AAAA} />}
          {r.MX && <ResultRow label={t('MX')} value={Array.isArray(r.MX) ? r.MX.map((m:any) => m.exchange || m).join(', ') : r.MX} />}
          {r.NS && <ResultRow label={t('NS')} value={Array.isArray(r.NS) ? r.NS.join(', ') : r.NS} />}
          {r.TXT && <ResultRow label={t('TXT')} value={Array.isArray(r.TXT) ? r.TXT.join(' | ') : r.TXT} />}
          {r.CNAME && <ResultRow label={t('CNAME')} value={Array.isArray(r.CNAME) ? r.CNAME.join(', ') : r.CNAME} />}
          {r.SOA && <ResultRow label={t('SOA')} value={typeof r.SOA === 'object' ? `${r.SOA.nsname} (${r.SOA.hostmaster})` : r.SOA} />}
    ...
    if (activeTab === 'whois') {
      return (
        <div>
          ...
          <ResultRow label={t('Domain')} value={r.domain_name || r.domainName || query} color="#FFD700" />
          <ResultRow label={t('Registrar')} value={r.registrar} />
          <ResultRow label={t('Created')} value={r.creation_date || r.createdDate} />
          <ResultRow label={t('Expires')} value={r.expiration_date || r.expiresDate} />
          <ResultRow label={t('Updated')} value={r.updated_date || r.updatedDate} />
          <ResultRow label={t('Status')} value={Array.isArray(r.status) ? r.status.join(', ') : r.status} />
          <ResultRow label={t('Nameservers')} value={Array.isArray(r.name_servers || r.nameServers) ? (r.name_servers || r.nameServers).join(', ') : r.name_servers} />
    ...
    if (activeTab === 'shodan') {
      return (
        <div>
          ...
          <ResultRow label={t('Target IP')} value={r.ip || query} color="#FF3D3D" />
          ...
          {r.tags?.length > 0 && <ResultRow label={t('Tags')} value={r.tags.join(', ')} color="#FF9500" />}
          {r.vulns?.length > 0 && (
            <div className="mt-2 p-2 border border-red-500/30 bg-red-500/10 rounded">
              <span className="text-[10px] font-mono text-red-400 font-bold mb-1 block">{t('Vulnerabilities')} ({r.vulns.length})</span>
              <div className="flex flex-wrap gap-1">
                ...
                {r.vulns.length > 10 && <span className="text-[9px] font-mono text-[#8A8880]">+{r.vulns.length - 10} {t('more')}</span>}
              </div>
            </div>
          )}
    ...
    if (activeTab === 'bgp') {
      return (
        <div>
          ...
          <ResultRow label={t('Query')} value={r.query} color="#00E5FF" />
          {r.type === 'ip' && r.ip && (
            <>
              {r.ip.prefixes?.map((p: any, i: number) => (
                <div key={i} className="mt-2 p-2 border border-[#00E5FF]/20 bg-[#00E5FF]/5 rounded">
                  <ResultRow label={t('ASN')} value={`AS${p.asn.asn} - ${p.asn.name}`} color="#00E5FF" />
                  <ResultRow label={t('Prefix')} value={p.prefix} />
                  <ResultRow label={t('Country')} value={p.asn.country_code} />
                  <ResultRow label={t('Description')} value={p.asn.description} />
                </div>
              ))}
            </>
          )}
          {r.type === 'asn' && r.asn && (
            <div className="mt-2 p-2 border border-[#00E5FF]/20 bg-[#00E5FF]/5 rounded">
              <ResultRow label={t('ASN')} value={`AS${r.asn.asn}`} color="#00E5FF" />
              <ResultRow label={t('Name')} value={r.asn.name} />
              <ResultRow label={t('Description')} value={r.asn.description} />
              <ResultRow label={t('Country')} value={r.asn.country_code} />
              {r.prefixes && <ResultRow label={t('Prefixes')} value={`IPv4: ${r.prefixes.total_v4} | IPv6: ${r.prefixes.total_v6}`} />}
              {r.peers && <ResultRow label={t('Peers')} value={r.peers.total} />}
            </div>
          )}
    ...
    if (activeTab === 'mac') {
      return (
        <div>
          ...
          <ResultRow label={t('MAC Address')} value={r.mac} color="#FFD700" />
          <ResultRow label={t('Vendor')} value={r.vendor} color={r.vendor === 'Not Found' ? '#FF3D3D' : '#00E676'} />
        </div>
      );
    }
    ...
    if (activeTab === 'phone') {
      return (
        <div>
          ...
          <ResultRow label={t('Query')} value={r.query} color="#FF9500" />
          <ResultRow label={t('Valid')} value={r.valid ? t('YES') : t('NO')} color={r.valid ? '#00E676' : '#FF3D3D'} />
          {r.valid && (
            <>
              <ResultRow label={t('E.164 Format')} value={r.number} />
              <ResultRow label={t('Intl Format')} value={r.international} />
              <ResultRow label={t('Nat Format')} value={r.national} />
              <ResultRow label={t('Country')} value={`${r.region} (${r.country_code})`} />
              <ResultRow label={t('Line Type')} value={r.line_type} color={r.line_type === 'MOBILE' ? '#00E5FF' : r.line_type === 'VOIP' ? '#FF9500' : undefined} />
            </>
          )}
        </div>
      );
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <div className="text-[9px] font-mono text-[var(--text-muted)]">@{r.username} • {r.followers} {t('Followers')}</div>
            </div>
          </div>
          <ResultRow label={t('Company')} value={r.company} />
          <ResultRow label={t('Location')} value={r.location} />
          <ResultRow label={t('Email')} value={r.email} color="#00E676" />
          <ResultRow label={t('Twitter')} value={r.twitter} color="#448AFF" />
          <ResultRow label={t('Website')} value={r.blog} />
          <ResultRow label={t('Bio')} value={r.bio} />
    ...
    if (activeTab === 'leaks') {
      return (
        <div>
          ...
          <ResultRow label={t('Email')} value={r.email} color="#E040FB" />
          <ResultRow label={t('Status')} value={r.breached ? t('COMPROMISED') : t('SECURE')} color={r.breached ? '#FF1744' : '#00E676'} />
          ...
          {r.breached && r.breaches?.length > 0 && (
            <div className="mt-2 p-2 border border-red-500/30 bg-red-500/10 rounded">
              <span className="text-[10px] font-mono text-red-400 font-bold mb-1 block">{t('KNOWN BREACHES')} ({r.breaches.length})</span>
    ...
    if (activeTab === 'certs') {
      ...
      return (
        <div>
          ...
          <ResultRow label={t('Domain')} value={query} color="#E040FB" />
          <ResultRow label={t('Certificates')} value={Array.isArray(certs) ? certs.length : 0} />
          {Array.isArray(certs) && certs.slice(0, 15).map((c: any, i: number) => (
            <div key={i} className="mt-1.5 p-2 rounded border border-[var(--border-secondary)]/30 bg-[var(--bg-tertiary)]/30">
              <ResultRow label={t('Issuer')} value={c.issuer_name || c.issuer} />
              <ResultRow label={t('Common Name')} value={c.common_name || c.name_value} />
              <ResultRow label={t('Not Before')} value={c.not_before} />
              <ResultRow label={t('Not After')} value={c.not_after} />
            </div>
          ))}
    ...
    if (activeTab === 'threats') {
      return (
        <div>
          ...
          <ResultRow label={t('Query')} value={query} color="#FF9500" />
          <ResultRow label={t('Risk Score')} value={r.risk_score || r.score} color={
            (r.risk_score || r.score || 0) > 70 ? '#FF3D3D' : (r.risk_score || r.score || 0) > 40 ? '#FF9500' : '#00E676'
          } />
          <ResultRow label={t('Malicious')} value={r.malicious !== undefined ? (r.malicious ? t('YES') : t('NO')) : undefined} color={r.malicious ? '#FF3D3D' : '#00E676'} />
          <ResultRow label={t('Category')} value={r.category || r.type} />
          <ResultRow label={t('Reports')} value={r.total_reports || r.reports} />
          <ResultRow label={t('Last Seen')} value={r.last_seen || r.last_analysis} />
          {r.tags && <ResultRow label={t('Tags')} value={Array.isArray(r.tags) ? r.tags.join(', ') : r.tags} />}
    ...
    if (activeTab === 'ssl') {
      return (
        <div>
          ...
          <ResultRow label={t('Target')} value={query} color="#76FF03" />
          <ResultRow label={t('Protocol')} value={r.protocol || r.tls_version} />
          <ResultRow label={t('Cipher')} value={r.cipher || r.cipher_suite} />
          <ResultRow label={t('Valid')} value={r.valid !== undefined ? (r.valid ? t('YES') : t('NO')) : undefined} color={r.valid ? '#00E676' : '#FF3D3D'} />
          <ResultRow label={t('Issuer')} value={r.issuer} />
          <ResultRow label={t('Subject')} value={r.subject} />
          <ResultRow label={t('Expires')} value={r.expires || r.not_after} />
          <ResultRow label={t('SANs')} value={Array.isArray(r.sans) ? r.sans.join(', ') : r.sans} />
          {renderFallback()}
        </div>
      );
    }
    ...
    <div className="flex flex-col gap-2.5">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-mono font-bold tracking-[0.1em] text-[11px]" style={{ color: '#00E676' }}>{loading ? `${t('TRACKING')}...` : t('SELF TRACK')}</span>
            </div>
          </button>
        </div>
        {/* Other Tools */}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {activeTab === 'sweep' && (
          <div className="flex items-center justify-between bg-[var(--bg-primary)]/60 border border-[var(--border-primary)] rounded-lg p-1">
            <span className="text-[9px] font-mono text-[var(--text-muted)] pl-2">{t('SUBNET MASK:')}</span>
      ...
      {sweepProgress && loading && (
        <div className="p-3 rounded-lg border border-[#FF3D3D]/30 bg-[#FF3D3D]/5">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] font-mono text-[#E8E6E0]">{sweepProgress.total} {t('hosts')}</span>
          </div>
      ...
      {sweepResult && !loading && activeTab === 'sweep' && (
        <div className="bg-[var(--bg-primary)]/40 border border-[var(--border-primary)] rounded-lg overflow-hidden max-h-[55vh] overflow-y-auto styled-scrollbar">
          ...
          <div className="p-3 border-b border-[#2A2A28]">
            ...
            >
              <Globe className="w-4 h-4" /> {t('VISUALIZE ON GLOBE')}
            </button>
          </div>
          {/* Device List */}
                ...
                {!isFullScreen && (
                  <>
                    <div className="flex items-center gap-2 text-[9px] font-mono text-[#5C5A54]">
                      <span>{t('Ports')}: {device.ports.slice(0, 8).join(', ')}{device.ports.length > 8 ? ` +${device.ports.length - 8}` : ''}</span>
                      {device.vulns.length > 0 && (
                        <div className="group relative flex items-center gap-1 cursor-help">
                          <span className="text-[#FF3D3D] flex items-center gap-1">
                            <AlertTriangle className="w-2.5 h-2.5" /> {device.vulns.length} {t('CVEs')}
                          </span>
                  ...
                  <div className="border-t border-[#2A2A28]">
                    ...
                    {device.vulns.length > 0 && (
                      <div className="p-4 border-t border-[#2A2A28]">
                        <div className="text-[10px] font-mono text-[#5C5A54] tracking-widest uppercase mb-3">{t('Vulnerabilities')} ({device.vulns.length})</div>
                              ...
                              <div key={cveId} className="bg-[#111] border border-[#2A2A28] rounded-lg p-3">
                                ...
                                {isLoading ? (
                                  ...
                                  <>
                                    ...
                                    {info.cwe && <div className="text-[10px] font-mono text-[#5C5A54] mt-2">{t('Weakness')}: {info.cwe}</div>}
          ...
          <div className="px-3 py-2 border-t border-[#2A2A28]">
            <div className="text-[8px] font-mono text-[#5C5A54] tracking-wider">{t('SWEPT')} {sweepResult.summary.total_hosts} {t('HOSTS')} {t('IN')} {(sweepResult.sweep_time_ms / 1000).toFixed(1)}s · ASN {sweepResult.center.asn}</div>
          </div>
        </div>
      )}
      ...
      {results && !(sweepResult && !loading) && (
        <div className="bg-[var(--bg-primary)]/40 border border-[var(--border-primary)] rounded-lg p-3 max-h-[50vh] overflow-y-auto styled-scrollbar">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[9px] font-mono tracking-widest" style={{ color: currentTab?.color }}>{currentTab?.label} {t('RESULTS')}</span>
      ...
      <div className="fixed top-4 bottom-4 right-4 w-[40vw] min-w-[600px] max-w-[800px] z-[999] glass-panel bg-[#0a0a09]/95 backdrop-blur-2xl border border-[var(--cyan-primary)]/40 rounded-xl flex flex-col overflow-hidden shadow-2xl shadow-[var(--cyan-primary)]/20 transition-all duration-300">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-secondary)] bg-[#111]">
          <div className="flex items-center gap-3">
            ...
            <span className="hud-text text-[16px] text-[var(--text-primary)]">{t('GÖKSEL RECON TOOLKIT')}</span>
