...
const UptimeClock = () => {
  ...
  const { t: ut } = useI18n();
  return <span className="hidden lg:inline">{ut('UPTIME')}: <span className="text-[var(--gold-primary)]">{uptime}</span></span>;
};
...
const ZuluClock = () => {
  ...
  const { t: zt } = useI18n();
  return <span className="text-[var(--cyan-primary)] font-bold tabular-nums">{time || `${zt('ZULU')} --:--:--Z`}</span>;
};
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
        ...
        {showSplash && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-80 h-50 mb-8 z-[2] overflow-hidden rounded-lg"
            >
              <video
                autoPlay
                loop
              ...
              >
                <source src="/bayrak.mp4" type="video/mp4" />
              </video>
            </motion.div>
            ...
            >
              <h1 className="text-4xl md:text-5xl font-bold tracking-[0.3em] font-mono"
                  style={{ color: 'var(--text-heading)', textShadow: '0 0 30px rgba(212,175,55,0.2)' }}>
                GÖKSEL
              </h1>
              <p className="text-[10px] md:text-[11px] font-mono tracking-[0.2em] text-[var(--gold-primary)] mt-3" style={{ opacity: 0.85 }}>
                TÜRKÇE AÇIK KAYNAK İSTİHBARATI
              </p>
            </motion.div>

            {/* ── Simple progress bar ── */}
            <div className="w-64 md:w-80 mt-10 z-[2]">
              <div className="relative w-full h-[2px] rounded-full overflow-hidden" style={{ background: 'rgba(212,175,55,0.1)' }}>
                <motion.div
                  initial={{ width: '0%' }}
                  animate={{ width: '100%' }}
                  transition={{ duration: 9, delay: 0.5, ease: 'easeInOut' }}
        ...
        <div className="flex items-center gap-1.5 pointer-events-auto">
          ...
          <div className="flex items-center rounded-xl overflow-hidden border border-[var(--border-primary)] bg-[var(--bg-panel)] backdrop-blur-2xl shadow-[0_4px_24px_rgba(0,0,0,0.5)]">
            ...
            >
              ...
              <span className="hidden md:inline">{t('3D')}</span>
            </button>
            ...
            >
              ...
              <span className="hidden md:inline">{t('2D')}</span>
            </button>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <div className="flex flex-col items-start gap-0.5">
            ...
            <span className="text-[8px] md:text-[9px] font-mono tracking-[0.2em] opacity-80 uppercase text-[#D4AF37]">{t('GLOBAL INTELLIGENCE COMMAND')}</span>
          </div>
        </div>
        <div className="flex items-center gap-3 mt-1.5 pl-[44px] min-w-0 pr-4">
          <span className="text-[5px] md:text-[6px] text-[var(--text-muted)] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-40 truncate">
            {t('POWERED BY GÖKSEL OPEN SOURCE INTELLIGENCE')} <span className="hidden md:inline">· {t('C2 ENGINE: PHYSICAL COMMAND CORE')} · {t('SENSORS: ORBITAL LATTICE')} · {t('NET: LYCAN NETWORK')}</span>
          </span>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-4 text-[9px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <span className="flex items-center gap-1">{t('SYS')}: <span className={backendStatus === 'connected' ? 'text-[var(--alert-green)]' : 'text-[var(--alert-red)]'}>{t(backendStatus.toUpperCase())}</span></span>
        ...
        <UptimeClock />
        <span className="text-[10px] font-bold tracking-[0.2em] text-[var(--text-muted)] opacity-50 ml-2">{t('V.4.1')}</span>
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[9px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? t('LAYERS & STATS') : mobilePanel === 'markets' ? t('MARKETS & INTEL') : mobilePanel === 'intel' ? t('INTEL FEED') : mobilePanel === 'recon' ? t('GÖKSEL RECON') : t('SEARCH')}
                    </span>
                    ...
                    <>
                      <div className="glass-panel-sm p-2 mb-2">
                        <div className="grid grid-cols-5 gap-1 text-center">
                          <div><div className="hud-label" style={{fontSize:'6px'}}>{t('AIR')}</div><div className="hud-value text-[9px]">{totalFlights.toLocaleString()}</div></div>
                          <div><div className="hud-label" style={{fontSize:'6px'}}>{t('SAT')}</div><div className="hud-value text-[9px]">{(data.satellites?.length||0)}</div></div>
                          <div><div className="hud-label" style={{fontSize:'6px'}}>{t('CAM')}</div><div className="hud-value text-[9px]">{(data.cameras?.length||0)}</div></div>
                          <div><div className="hud-label" style={{fontSize:'6px'}}>{t('WX')}</div><div className="hud-value text-[9px]" style={{color:'var(--accent-weather)'}}>{(data.weather_events?.length||0)}</div></div>
                          <div><div className="hud-label" style={{fontSize:'6px'}}>{t('NUC')}</div><div className="hud-value text-[9px]" style={{color:'var(--accent-nuclear)'}}>{(data.infrastructure?.length||0)}</div></div>
                        </div>
                      </div>
